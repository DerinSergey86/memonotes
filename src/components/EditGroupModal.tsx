...
export default function EditGroupModal({ group, onSave, onClose }: EditGroupModalProps) {
  ...
  const [tagsString, setTagsString] = useState(group.tags.slice(1).join(', '));
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const relatedTags = tagsString.split(',').map(t => t.trim()).filter(t => t !== '');
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000 }}>
      <div style={{ background: 'white', padding: '20px', borderRadius: '8px', width: '90%', maxWidth: '400px', boxShadow: '0 4px 12px rgba(0,0,0,0.15)' }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom: '12px' }}>
            ...
            <input type="text" value={tagsString} onChange={e => setTagsString(e.target.value)} style={{ width: '100%', marginTop: '4px' }} />
          </div>
          <div style={{ marginBottom: '12px' }}>
            <label>Картинка (URL или файл):</label>
            <input type="text" value={image} onChange={e => setImage(e.target.value)} style={{ width: '100%', marginTop: '4px' }} placeholder="URL" />
            <input type="file" accept="image/*" onChange={handleFileChange} style={{ marginTop: '4px' }} />
            {image && <img src={image} alt="preview" style={{ width: '100%', height: '80px', objectFit: 'cover', marginTop: '4px', borderRadius: '4px' }} />}
          </div>
